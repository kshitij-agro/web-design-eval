...
def render_html_to_png(html: str, png_path: Path) -> None:
    """Render `html` to `png_path` at the canonical viewport, full-page.

    We write the HTML to a sibling temp file and load it via file:// because
    Google Fonts links inside the HTML are easier to fetch from a file context
    than from a data: URL (which has tighter origin handling in Chromium).
    """
    png_path = png_path.resolve()
    png_path.parent.mkdir(parents=True, exist_ok=True)
    html_path = png_path.with_suffix(".source.html")
    ...
    # Keep the sibling HTML around — useful for debugging and oracle solution.
